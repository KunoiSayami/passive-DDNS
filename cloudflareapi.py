...
class DNSRecord:
    ...
    def update_dns(self, session: requests.Session, change_to_ip: str, retries: int = 3) -> None:
        ...
        try:
            r = session.put(f'https://api.cloudflare.com/client/v4/zones/{self.zone_id}/dns_records/{self.id}',
                            json=self.get_params(), timeout=30)
            r.raise_for_status()
            r.close()
...
class CloudFlareApi:
    ...
    def get_domain_record(self, domain: str, name: str, retries: int = 3) -> Dict[str, FixedType]:
        time.sleep(retry_delay[retries])
        try:
            r = self.session.get(f'https://api.cloudflare.com/client/v4/zones/{domain}/dns_records',
                                 params={'type': 'A', 'name': name}, timeout=30)
            r.raise_for_status()
